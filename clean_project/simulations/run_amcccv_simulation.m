function results = run_amcccv_simulation(config, BatteryQueue, PV_profile)
%RUN_AMCCCV_SIMULATION Swap-station simulation using AM-CC-CV charging.
%   results = RUN_AMCCCV_SIMULATION(config, BatteryQueue, PV_profile)
%   simulates the station using an adaptive-multi-stage constant-current /
%   constant-voltage (AMCCCV) charging policy instead of MPC, for
%   comparison against the MPC, LP, and baseline simulations.
%
%   Inputs:
...
%% ================= HELPER FUNCTIONS (AMCCCV) =================

    function P_grid = charging_profile(B)
        % AMCCCV grid-side power (W), SoC-based stages
        ...
        I_max = 2.6;              % YOUR value
        V_min = 0.85 * V_ref;
        V_max = 1.20 * V_ref;

        % --- Multi-stage CC logic (unchanged conceptually) ---
        if SOC < 0.2
            I = 0.6 * I_max;
        elseif SOC < 0.6
            I = 1.0 * I_max;
    ...
    function B = charge_battery(P_grid, B)
        % Same battery model as baseline
        ...
        Cap_Wh = V * Ah;
        E_batt_Wh = P_grid * dt_h * config.Charger.Efficiency;

        B.SoC = min(B.SoC + E_batt_Wh / Cap_Wh, 1);
    end
